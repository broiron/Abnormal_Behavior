...
# tracks 삭제
def draw_tracks(frame, customerList, show_flow=False, show_cov=False,
                show_traj=False):
    '''
	for track in tracks:
    ...
    for key in customerList:
        draw_bbox(frame, customerList[key].tlbr, get_color(customerList[key].customerId), 2, str(customerList[key].customerId))
...
def draw_bbox(frame, tlbr, color, thickness, text=None):
    ...
    if text is not None:
        (text_width, text_height), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_DUPLEX, 2, 1)
...
class Visualizer:
    ...
    def render(self, frame, customerList, detections, klt_bboxes, prev_bg_keypoints, bg_keypoints):
        """Render visualizations onto the frame."""
        draw_tracks(frame, customerList, show_flow=self.draw_obj_flow,
