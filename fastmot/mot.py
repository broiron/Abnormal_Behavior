...
class MOT:
    def __init__(self, size, detections, 
                 detector_type='YOLO',
    ...
    def step(self, frame):
        ...
        self.timestamps = self.tracker.timestamp
        self.detections = detections
        if self.draw:
    ...
    def _draw(self, frame, detections, customerList):
        visible_tracks = list(self.visible_tracks())
        self.visualizer.render(frame, customerList, detections, self.tracker.klt_bboxes.values(),
